    ...
    public ApiResponse<T> create(Response<T> response){
        ...
        if(response.isSuccessful()){
            ...
            if(body instanceof ListResponse){
                if(!CheckModelApiKey.isModelApiKeyValid((ListResponse) body)){
                    String errorMsg = "Api key is invalid or expired.";
                    return new ApiErrorResponse<>(errorMsg);
                }
                if(((ListResponse) body).getCount() == 0){
                    // query is exhausted
                    return new ApiErrorResponse<>(QUERY_EXHAUSTED);
                }
            }

            if(body instanceof ItemResponse){
                if(!CheckModelApiKey.isModelApiKeyValid((ItemResponse) body)){
                    String errorMsg = "Api key is invalid or expired.";
                    return new ApiErrorResponse<>(errorMsg);
                }
            }
